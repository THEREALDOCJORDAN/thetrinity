...
function App() {
  ...
  const renderCurrentMode = () => {
    switch (currentMode) {
      ...
      case 'admin_dashboard':  return <AdminDashboard sessionId={sessionId} />;
      case 'discovery_lab':    return <DiscoveryLab sessionId={sessionId} />;
      ...
      <div className="dashboard-interface">
        <div className="store-selector">
          ...
          <select value={selectedStore?.shop_domain || ''} onChange={(e) => {
            ...
            {stores.map(store => (
              <option key={store.id} value={store.shop_domain}>{store.name}</option>
            ))}
          </select>
        </div>
